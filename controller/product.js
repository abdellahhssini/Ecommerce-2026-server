...
export const updatedProduct = TryCatch(async(req, res) => {
    ...
    const updatedProduct = await Product.findByIdAndUpdate(req.params.id, updatedFields, { new: true, runValidators: true });

    if (!updatedProduct) return res.status(403).json({ message: "Product not found", });
